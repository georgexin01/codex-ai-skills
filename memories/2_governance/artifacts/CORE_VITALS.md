...
- **Tokens are Debt**: Minimize cloud reasoning. Use `Delta-Only` tokens (read summaries first).
- **Reasoning Tiers**:
    - **Director**: Gemini 3 Flash (Fast Scaffold, UI, Tool Orchestration).
    - **Architect**: Claude Sonnet (Complex TypeScript architecture, code review, security audit).
    - **Executor**: OpenAI GPT-4o (Structured JSON/SQL, batch output generation).
    - **Local**: Gemma-4 (Local Boilerplate, File-Edits, Heavy Data).
...
- **Typography**: Plus Jakarta Sans (App), Inter + Orbitron (Web).
