...
# Task Group: supabase-project-backup-restore migration-readiness, VPS backup flow, and mixed-case pg_dump debugging
scope: Use when the user wants this backup workspace explained, needs local Angel migration-readiness checks, or is debugging VPS schema backup failures such as mixed-case `pg_dump` issues.
...
## Task 3: Explain the workspace and `vps-backup.bat`, success
### rollout_summary_files
- rollout_summaries/2026-06-08T07-00-00-wfVd-supabase_vps_backup_failed_on_mixed_case_schema_pgdump.md (cwd=\\?\C:\Users\user\Documents\supabase-project-backup-restore, rollout_path=C:\Users\user\.codex\sessions\2026\06\08\rollout-2026-06-08T15-00-05-019ea607-f8d7-7801-8fa8-bf8e4a70b357.jsonl, updated_at=2026-06-08T07:04:06+00:00, thread_id=019ea607-f8d7-7801-8fa8-bf8e4a70b357, mapped the Windows launcher, Git Bash wrapper, VPS script, and local backup destination)
### keywords
- vps-backup.bat, scripts/02-backup-vps.sh, scripts/vps-side/project-backup.sh, vps-backups, scp, ssh, Git Bash, manifest
## Task 4: Diagnose failed `quizLaa` VPS backup and patch mixed-case schema quoting, partial
### rollout_summary_files
- rollout_summaries/2026-06-08T07-00-00-wfVd-supabase_vps_backup_failed_on_mixed_case_schema_pgdump.md (cwd=\\?\C:\Users\user\Documents\supabase-project-backup-restore, rollout_path=C:\Users\user\.codex\sessions\2026\06\08\rollout-2026-06-08T15-00-05-019ea607-f8d7-7801-8fa8-bf8e4a70b357.jsonl, updated_at=2026-06-08T07:04:06+00:00, thread_id=019ea607-f8d7-7801-8fa8-bf8e4a70b357, traced `pg_dump: error: no matching schemas were found` to mixed-case schema quoting and patched both VPS/local dump helpers)
### keywords
- quizLaa, pg_dump: error: no matching schemas were found, PG_DUMP_SCHEMA_PATTERN, mixed-case schema, dump-schema.sh, [1/10] Schema DDL...
## User preferences
...
- when the user asks what a backup workspace or script does, explain the real source -> backup artifact -> local destination flow, not just filenames [Task 3]
- when the user asks "where i failed and how i failed and how to solve this" and pastes terminal output -> anchor the diagnosis on the exact runtime log and the first failing command [Task 4]
## Reusable knowledge
...
- `vps-backup.bat` is only a Windows launcher; the actual flow is `.bat` -> `scripts/02-backup-vps.sh` -> upload/run `scripts/vps-side/project-backup.sh` over `scp`/`ssh` -> download into `vps-backups/<schema>-<timestamp>` -> remove the remote temp folder [Task 3]
- The VPS backup includes schema/data SQL, public metadata, auth rows, storage metadata, grants, storage policies, public functions, edge functions, storage files, and a manifest; `scripts/lib/pick-schema.sh` still reads schema names from the local Docker Supabase container [Task 3]
- Mixed-case PostgreSQL schemas can resolve in `psql` but fail in `pg_dump -n` unless the schema pattern is passed as an embedded quoted identifier; the fix here was `PG_DUMP_SCHEMA_PATTERN=\"\\\"$PG_SCHEMA\\\"\"` in the VPS script and the same quoting pattern in `scripts/lib/dump-schema.sh` [Task 4]
## Failures and how to do differently
...
- `.env` in this workspace can hold live VPS credentials; never repeat them in memory or user-facing output [Task 3]
- If a future VPS backup dies at `[1/10] Schema DDL...` with `pg_dump: error: no matching schemas were found`, check uppercase letters in the schema name and inspect the exact `pg_dump -n` argument before chasing SSH or project-resolution issues; this rollout patched the likely fix but did not capture a live rerun [Task 4]
